...
- 完备质量体系：69 项自动化测试 + 20 题评测集（recall@4 = 1.0）
...
```
智能客服助手/
├── app.py                  # Streamlit 入口（侧边栏导航 + 按需渲染）
...
└── 测试脚本/               # 测试与评测
    ...
    ├── eval_recall.py      # recall@k 评测脚本
    └── eval_baseline.txt   # 评测基线记录
```
...
```bash
# 1. 安装依赖（Python 3.10 + 虚拟环境，清单见 requirements.txt）
pip install -r requirements.txt

# 2. 创建 .env 密钥文件
# 本项目约定 .env 放在项目根目录 RAGProject/（app.py 的上一级）
# .env内容：
# DEEPSEEK_API_KEY=你的key
# OPENAI_API_BASE_URL=https://api.deepseek.com/v1

# 提示：如果想把.env放在应用目录内，需要修改代码中读取dotenv的文件路径
...
| 检索阈值拍脑袋设置 | 用真实语料**实测标定**（相关 ≥0.44 / 无关 ≤0.11，取 0.30），并用评测集做敏感性实验 | 阈值 0.50 时 recall 降至 0.35，失败题分析定位为阈值误杀 |
...
| 改代码/调参导致效果退化 | **69 项功能测试 + 20 题评测集（recall@4=1.0）**双重回归 | 全量测试与评测脚本一键重跑 |
